Litenode Core version *0.15.1* is now available from:

  <https://download.litenode.org/litenode-0.15.1/>
...
A number of changes to the way Litenode Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 litenodes per kB (0.00001 LTN). This is to prepare for dropping the minimum transaction fee to 0.1 litenodes per kB (0.0001 LTN) in the next release.
